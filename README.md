...
**2026-09-14: `FIRMWARE_VERSION` manually reset to `4.0.0`.** v4.0.0b
through v4.4.0b below were the beta/development cycle for this project;
the `b` suffix marks that they were never a real released version number
(the running firmware never actually reported itself as, say, "4.2.3" --
that only ever existed as this table's own bookkeeping during
development). Versioning restarts clean from v4.0.0 below the beta rows.
