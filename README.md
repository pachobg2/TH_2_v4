...
- A **"Device status"** box:
  ...
  - **IP config** — the saved static IP/gateway/subnet/DNS if [static IP
    is in use](#network-settings-static-ip--bssid), or "DHCP" otherwise.
  - **BSSID pin** — the saved BSSID if one's pinned, or "none (any AP)".
...
**Declared stable, renumbered to a clean v4.0.0.** Everything through
v4.10.0b below was the beta/development cycle, and the `b` suffix meant
`FIRMWARE_VERSION` genuinely reported that suffixed string (over MQTT, in
HA, on the portal page), not just this table's bookkeeping. Briefly
declared stable at v4.10.0 (dropping just the `b`) before this final
renumbering to `4.0.0` -- a deliberate relabeling of that same build, not
a new one; nothing in the code changed between v4.10.0 and this v4.0.0.
This is distinct from the *other* `4.0.0` in this table's history: an
earlier attempt to declare it stable at a clean `4.0.0` (see the now-
relabeled v4.4.1b-v4.4.3b below) turned out to be premature -- real bugs
kept surfacing on hardware right after -- so it was walked back within
the same day. This time the `4.0.0` baseline is deliberate and confirmed,
after a full regression pass with no b-suffix history left to walk back.
...
| v4.2.0 | 2026-09-26 | New **Update Interval** select entity in HA (1, 2, 5, 10, 15, 30 or 60 min; default 10) choosing how long the device sleeps between readings. It is a retained, NVS-persisted command picked up on the next wake, so a change from HA takes up to one old interval to arrive and then applies to the sleep that follows; the current value is echoed back as state. `expire_after` on every HA entity now follows the chosen interval (3x) instead of the compile-time `EXPIRE_AFTER_SEC`, which is removed from `config.h`; when the interval changes, discovery is re-sent immediately so entities do not flap unavailable. `SLEEP_INTERVAL_US` in `config.h` is now just the default. |
